...
![1635820109754](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/02/102830-189901.png)
...
```java
HashMap<Integer, Integer> keyToFreq;//记录每一个key的访问频率
```
...
```java
//key代表多个访问频次相同的值，value是一个set存储访问频次相同的key
...
LFU 的逻辑不难理解，但是写代码实现并不容易，因为你看我们要维护`KV`表，`KF`表，`FK`表三个映射，特别容易出错。对于这种情况，教你三个技巧：
...
![1635820494504](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/02/103455-981173.png)
