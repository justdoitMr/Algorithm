...
![1635555492839](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202110/30/085813-868513.png)
        ...
        for(int i=1;i<=sz;i++){
            for(int w=1;w<=bin;w++){
                if(w-nums[i-1] <0){
                    ...
                    dp[i][w]=Math.max(dp[i - 1][w - nums[i-1]] + nums[i-1], dp[i - 1][w] );
                }
            }
        }
        return dp[sz][sum/2] ==sum/2?true:false;
    }
}
~~~
